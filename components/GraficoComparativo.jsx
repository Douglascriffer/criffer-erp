...
function InsideLabel({ x, y, width, height, value }) {
  ...
  if (inside) return <text x={x+width-8} y={y+height/2+4} fill="white" fontSize={12} fontWeight={700} textAnchor="end">{fmtV(value)}</text>
  return <text x={x+width+8} y={y+height/2+4} fill="#555" fontSize={12} fontWeight={700} textAnchor="start">{fmtV(value)}</text>
}
...
export default function GraficoComparativo({ currentData, previousData, currentLabel, previousLabel, showComparison=true }) {
  ...
  if (!showComparison || !previousData) {
    const data = CATS.map(cat => ({
      cat,
      val: currentData[cat.toLowerCase().replace('ç','c')] || currentData[cat==='Serviços'?'servicos':cat.toLowerCase()] || 0,
    ...
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={data} layout="vertical" margin={{ top:4, right:90, left:70, bottom:4 }} barCategoryGap="30%">
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        ...
        {chartData.map(d => {
          ...
          return (
            <div key={d.cat} style={{ background:'#F9F9F9', borderRadius:10, padding:'10px 8px', textAlign:'center' }}>
              ...
              <span style={{ fontSize:14, fontWeight:900, color:diff>=0?'#16a34a':'#EF4444', fontFamily:'Syne,sans-serif' }}>
      ...
      {/* Horizontal bars — NO grid */}
      <ResponsiveContainer width="100%" height={210}>
        <BarChart data={chartData} layout="vertical" margin={{ top:4, right:90, left:70, bottom:4 }} barCategoryGap="25%" barGap={3}>
          <XAxis type="number" hide/>
          <YAxis type="category" dataKey="cat" tick={{ fontSize:13, fill:'#555', fontWeight:600 }} axisLine={false} tickLine={false}/>
          <Tooltip content={<Tip/>}/>

...
export default function TVMode({ data }) {
  ...
  return (
    <div style={{ 
      ...
      {/* Conteúdo Principal (Slides) */}
      <div style={{ flex: 1, padding: '20px 60px 60px', position: 'relative' }}>
        {currentSlide === 0 && <SlideReceitas data={data} t={t} />}
        {currentSlide === 1 && <SlideMapa data={data} t={t} />}
        {currentSlide === 2 && <SlideVendedores data={data} t={t} />}
        {currentSlide === 3 && <SlideMetas data={data} t={t} />}
        {currentSlide === 4 && <SlideOrcamento data={data} t={t} />}
      </div>

      {/* Footer / Ticker */}
      <div style={{ 
        padding: '20px 60px', 
        background: 'rgba(255,106,34,0.05)', 
        ...
        <div style={{ display: 'flex', gap: 40 }}>
           ...
           <TickerItem label="SISTEMA" value="v4.2 PRO" />
        </div>
...
// ── TELA 1: RECEITAS ──
function SlideReceitas({ data, t }) {
  const receitas = data?.receitas || {}
  const totalReceita = (receitas.vendas || 0) + (receitas.servicos || 0) + (receitas.locacao || 0)
  const meta = 2200000 // Exemplo de meta mensal
  const pct = (totalReceita / meta) * 100
  ...
  return (
    <div className="slide-enter" style={{ display: 'flex', flexDirection: 'column', gap: 40, height: '100%' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 30 }}>
        <KpiCardTV label="VENDAS" value={receitas.vendas || 0} icon={ArrowUpRight} color={t.accent} t={t} />
        <KpiCardTV label="SERVIÇOS" value={receitas.servicos || 0} icon={Activity} color={t.accent} t={t} />
        <KpiCardTV label="LOCAÇÃO" value={receitas.locacao || 0} icon={TrendingUp} color={t.accent} t={t} />
        <KpiCardTV label="DEVOLUÇÃO" value={receitas.devolucao || 0} icon={ArrowDownRight} color={t.red} t={t} />
      </div>
      ...
      <div style={{ flex: 1, background: t.card, borderRadius: 32, border: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', padding: '0 60px', gap: 80 }}>
        <div style={{ flex: 1 }}>
           ...
           <p style={{ fontSize: 120, fontWeight: 900, margin: 0, lineHeight: 1, letterSpacing: -4 }}>{fmt(totalReceita)}</p>
        ...
        <div style={{ width: 400, height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
           ...
           <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={[
                    { name: 'Vendas', value: receitas.vendas },
                    { name: 'Outros', value: (receitas.servicos + receitas.locacao) }
                  ]}
                  innerRadius={140}
                  outerRadius={180}
                  paddingAngle={5}
...
// ── TELA 3: VENDEDORES ──
function SlideVendedores({ data, t }) {
  const ranking = [
  ...
  return (
    <div className="slide-enter" style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 40 }}>
      <div style={{ flex: 1, background: t.card, borderRadius: 32, border: `1px solid ${t.border}`, padding: 60 }}>
        <h3 style={{ fontSize: 28, fontWeight: 900, color: t.accent, textTransform: 'uppercase', marginBottom: 40 }}>Ranking Performance Mensal</h3>
        <ResponsiveContainer width="100%" height="100%">
  ...
  return (
    <div className="slide-enter" style={{ height: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40 }}>
      <div style={{ background: t.card, borderRadius: 32, border: `1px solid ${t.border}`, padding: 50 }}>
         <h3 style={{ fontSize: 24, fontWeight: 900, color: t.accent, textTransform: 'uppercase', marginBottom: 40 }}>Evolução Acumulada</h3>
         <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={hist}>
              <defs>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 30 }}>
         <div style={{ background: t.accent, borderRadius: 32, padding: 50, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <p style={{ fontSize: 18, fontWeight: 800, color: '#000', marginTop: 10 }}>↓ Faltam 12% para bater a meta anual</p>
         </div>
         <div style={{ background: t.card, borderRadius: 32, border: `1px solid ${t.border}`, padding: 50, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <p style={{ fontSize: 20, fontWeight: 900, color: t.textMuted, textTransform: 'uppercase', margin: 0 }}>Performance Mensal</p>
            <p style={{ fontSize: 80, fontWeight: 900, color: t.green, margin: 0 }}>+18.4%</p>
            <p style={{ fontSize: 18, fontWeight: 800, color: t.green, marginTop: 10 }}>↑ Acima do projetado para o período</p>
         </div>
      </div>
    </div>
  )
...
// ── TELA 5: ORÇAMENTO ──
function SlideOrcamento({ data, t }) {
  return (
    <div className="slide-enter" style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 40 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, flex: 1 }}>
         <div style={{ background: t.card, borderRadius: 32, border: `1.5px solid ${t.border}`, padding: 60, display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 24, fontWeight: 800, color: t.green }}>POSITIVO</p>
         </div>
...
function KpiCardTV({ label, value, icon: Icon, color, t }) {
  return (
    <div style={{ 
      ...
      <div style={{ background: color, padding: 15, borderRadius: 16 }}>

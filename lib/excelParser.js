    ...
    reader.onload = (e) => {
      try {
        ...
        if (workbook.SheetNames.includes('COMERCIAL')) {
          ...
          if (rows.length > 1) {
            const headers = rows[0] // Linha de cabeçalho com nomes dos vendedores
            const sellers = []
            ...
            for (let i = 1; i < rows.length; i++) {
              ...
              // Percorre as colunas de vendedores (B até P/Q)
              // Vamos parar antes da coluna 'Total' (que costuma ser a última preenchida significativamente)
              headers.forEach((header, colIdx) => {
                if (colIdx === 0) return // Pula a coluna de data
                if (header === 'Total' || header === 'Realizado' || !header) return

        ...
        if (workbook.SheetNames.includes('COMERCIAL')) {
          ...
          if (rows.length > 1) {
            ...
            for (let i = 1; i < rows.length; i++) {
              ...
              // Pega o 'Realizado' da coluna R (índice 17)
              ...
              headers.forEach((header, colIdx) => {
                if (colIdx === 0) return 
                let hName = String(header || '').trim()
                
                // Coluna P (índice 15) - Vendedor não identificado
                if (colIdx === 15 && !hName) {
                  hName = 'Vendedor não identificado'
                }

                // Pula colunas de totais (Q e R)
                if (colIdx >= 16 || !hName) return

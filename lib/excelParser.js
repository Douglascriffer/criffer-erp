  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // ── Sheet: COMERCIAL (Seller performance detail) ────────
        if (workbook.SheetNames.includes('COMERCIAL')) {
          const ws   = workbook.Sheets['COMERCIAL']
          const rows = XLSX.utils.sheet_to_json(ws, { defval: 0 })
          result.bySeller = rows.map(row => {
            const dt = row['Data'] instanceof Date ? row['Data'] : new Date(row['Data'])
            return {
              ano:      dt.getFullYear() || 2026,
              mes:      dt.getMonth() + 1 || 1,
              vendedor: row['Vendedor'] || row['Nome'] || row['VENDEDOR'] || 'Desconhecido',
              valor:    Number(row['Valor'] || row['Faturamento'] || row['Venda'] || row['VALOR'] || 0)
            }
          }).filter(r => r.valor !== 0)
        }

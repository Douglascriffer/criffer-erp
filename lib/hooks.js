...
export function useFilteredData(data, filters) {
  ...
  useEffect(() => {
    ...
    const totalVendas      = byPeriod.reduce((s, r) => s + r.vendas, 0)
    const totalServicos    = byPeriod.reduce((s, r) => s + r.servicos, 0)
    const totalLocacao     = byPeriod.reduce((s, r) => s + r.locacao, 0)

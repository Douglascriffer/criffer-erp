...
export function useFilteredData(data, filters) {
  ...
  useEffect(() => {
    ...
    // KPIs
    const totalFaturamento = byPeriod.reduce((s, r) => s + r.total, 0)
    const totalMeta        = metaData.reduce((s, r) => s + r.meta, 0)
    const totalRealizado   = metaData.reduce((s, r) => s + r.realizado, 0)
    const pctAtingido      = totalMeta > 0 ? (totalRealizado / totalMeta) * 100 : 0
    const vendas           = byPeriod.reduce((s, r) => s + r.vendas, 0)
    const servicos         = byPeriod.reduce((s, r) => s + r.servicos, 0)
    const locacao          = byPeriod.reduce((s, r) => s + r.locacao, 0)
    const devolucoes       = byPeriod.reduce((s, r) => s + r.devolucoes, 0)
